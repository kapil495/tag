...
3>   "/game/game_definers"  :=  contains all of the js files and configs for sprites, background , animations and scenes required for the game.js (present in /public) . for its internal structure go to [/game_definers](#game_definers)<br><br>
...
3.1 >  "/game/game_definers/scenes"  :=  contains all the scenes file for the game<br><br>
3.2 >  "/game/game_definers/statics"  :=  constains all the positions, paths , sprite sheet configs , etc which are to be used for animations and scenes. for more go to [/game_definers/statics](#/game_definers/statics)<br><br>
3.3 >  "/game/game_definers/load"  :=  contains all the functions for loading all the asssets.<br><br>
3.4 >  "/game/game_definers/animate"  :=  all the functions related to || loading || of animation of wither chaser or runner are there.<br><br>
3.5 >  "/game/game_definers/set"  :=  all the functions related to setting up the initial positions of background , chasers , runners , etc.<br><br>
3.6 >  "/game/game_definers/keys"  :=  all the states of keyboard key if pressend or not are present inside it. for aadding the new key for checking its state if pressed or not , add that key in "/game/game_definers/statics/keys.js". then it can be accesible by anywhere in code by<br>
//gameScene.js 
stateOf.__key
<br>
3.6 >  "/game/game_definers/animate"  :=  responsible for animation of chaser and runners . can be used by<br>
//gameScene.js
...
3.2.1 >  "/game/game_definers/statics/background.js"  :=  contains the list of possible backgroundsin form of array backggrounds . 
...
3.2.2 >  "/game/game_definers/statics/chasers.js"  :=  constain the list of all the chasers with their sprite sheet's frame width and height
...
3.2.3 >  "/game/game_definers/statics/chaserAnims.js"  :=  contains the parameter for the animation of chaser.🤞currently not developed this fully flexible.
...
3.2.4 >  "/game/game_definers/statics/runners.js"  :=  constain the list of all the runners with their sprite sheet's frame width and height 🤞currently not developed
...
3.7.1 >  "/game/game_definers/update/background.js"  :=  contains a f